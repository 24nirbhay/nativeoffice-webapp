...
def test_ppt(page: Page) -> None:
    ...
    page.get_by_role("button", name="Presentation Slides that land", exact=True).click()
    title = page.get_by_text("Click to add title|Add a title")
    title.click()
    title.dblclick()
    page.locator(".sl-editor").fill("hello")
    subtitle = page.get_by_text("Click to add subtitle|Add a subtitle")
    subtitle.click()
    subtitle.dblclick()
    page.locator(".sl-editor").fill("hello")
